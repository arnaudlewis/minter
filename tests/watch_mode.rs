...
impl LineReceiver {
    ...
    fn wait_for(&self, predicate: impl Fn(&str) -> bool, timeout: Duration) -> Option<String> {
...
fn watch_graceful_shutdown() {
    ...
    // Wait for process to exit — accept exit code 0 (graceful) or signal
    // termination by SIGINT (Linux default disposition can fire before handler completes)
    ...
    #[cfg(unix)]
    {
        ...
        assert!(
            ...
            "should exit cleanly or via SIGINT, got: {status}"
        );
    }
    #[cfg(not(unix))]
    assert!(
...
fn watch_revalidate_after_fix() {
    ...
    receiver
        .wait_for(|l| l.contains('\u{2717}'), Duration::from_secs(10))
    ...
    let success_line = receiver.wait_for(|l| l.contains('\u{2713}'), Duration::from_secs(10));
    assert!(
...
fn watch_color_changed_file_yellow() {
    ...
    let changed_line = receiver.wait_for(|l| l.contains("changed"), Duration::from_secs(10));
...
fn watch_color_deleted_file_red() {
    ...
    let deleted_line = receiver.wait_for(|l| l.contains("deleted"), Duration::from_secs(10));
...
fn watch_file_regresses_valid_to_invalid() {
    ...
    let cross_line = stdout_rx.wait_for(|l| l.contains('\u{2717}'), Duration::from_secs(10));
    assert!(
        cross_line.is_some(),
...
fn watch_dependency_removed() {
    ...
    let line = receiver.wait_for(|l| l.contains("a"), Duration::from_secs(10));
    assert!(
        line.is_some(),
...
fn watch_integrate_new_nfr_file() {
    ...
    let line = receiver.wait_for(
        |l| l.contains("reliability") || l.contains("new") || l.contains("detected"),
        Duration::from_secs(10),
    );

    assert!(line.is_some(), "should detect and report new .nfr file");
