...
fn nfr_with_rule_constraint(category: &str, constraint_name: &str, overridable: &str) -> String {
    format!(
        "\
nfr {category} v1.0.0
title \"{category} Requirements\"
...
fn reject_behavior_level_whole_file_ref() {
    ...
    minter()
        ...
        .assert()
        .failure()
        .stderr(predicates::str::contains("whole-file").and(predicates::str::contains("behavior")))
        .stderr(predicates::str::contains(path.to_str().unwrap()));
}
...
fn resolve_category_to_nfr_file() {
    ...
    let nfr_content =
        nfr_with_metric_constraint("performance", "api-response-time", "<", "1s", "yes");
    let (_dir, dir_path) =
        temp_dir_with_spec_and_nfrs("test-spec", &spec_content, &[("performance", &nfr_content)]);
...
fn reject_missing_nfr_category_file() {
    ...
    let (_dir, dir_path) = temp_dir_with_spec_and_nfrs("test-spec", &spec_content, &[]);
    minter()
        .arg("validate")
        .arg(&dir_path)
        .assert()
...
fn reject_missing_anchor() {
    ...
    minter()
        ...
        .failure()
        .stderr(
            predicates::str::contains("nonexistent").and(predicates::str::contains("performance")),
        )
        .stderr(predicates::str::contains("test-spec"));
}
...
fn reject_override_on_overridable_no() {
    ...
    let (_dir, dir_path) =
        temp_dir_with_spec_and_nfrs("test-spec", &spec_content, &[("security", &nfr_content)]);
    minter()
        .arg("validate")
        .arg(&dir_path)
        .assert()
...
fn reject_override_on_rule() {
    let spec_content = spec_with_behavior_nfr_override("performance", "no-n-plus-one", "<", "5");
