...
use common::{
    VALID_NFR, VALID_SPEC, minter, temp_dir_with_nested_specs, temp_dir_with_spec_and_nfrs,
    temp_dir_with_specs, temp_nfr, temp_spec, temp_specs,
};
...
behavior do-thing [happy_path]
  ...
  then emits stdout
    assert output contains \"done\"
";
    let (_dir, dir_path) = temp_dir_with_nested_specs(&[("sub/a", spec_a), ("sub/deep/b", spec_b)]);
    let output = minter().arg("validate").arg(&dir_path).assert().success();
...
";
    ...
    let output = minter().arg("validate").arg(&path).assert().failure();
...
";
    ...
    let output = minter().arg("validate").arg(&dir_path).assert().failure();
...
fn validate_file_exceeds_max_size() {
    ...
    std::fs::write(&path, content).expect("write huge file");
    minter()
        .arg("validate")
        .arg(&path)
        .assert()
        ...
        .stderr(predicate::str::contains(
            "exceeds maximum file size of 10MB",
        ));
}
...
fn reject_excessive_depth() {
    let dir = tempfile::TempDir::new().expect("create temp dir");
    // Create a chain of 258 specs: spec-0 -> spec-1 -> ... -> spec-257
    // This produces a recursion depth of 257 which exceeds the limit of 256.
    let chain_len = 258;
    for i in 0..chain_len {
        let depends_on = if i < chain_len - 1 {
            format!("\ndepends on spec-{} >= 1.0.0\n", i + 1)
        } else {
            String::new()
        };
        let content = format!(
...
{depends_on}"
        ...
        let path = dir.path().join(format!("spec-{i}.spec"));
        std::fs::write(&path, content).expect("write spec file");
    }
    let root = dir.path().join("spec-0.spec");
    minter()
        .arg("validate")
        .arg("--deep")
        .arg(&root)
        ...
        .failure()
        .stderr(predicate::str::contains(
            "dependency resolution exceeded maximum depth of 256",
        ));
}
