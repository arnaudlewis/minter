...
use common::{VALID_SPEC, minter, temp_dir_with_specs, temp_nfr, temp_spec, temp_specs};
...
fn display_success_line() {
    ...
    minter()
        ...
        .stdout(predicate::str::contains(
            "✓ my-feature v1.2.0 (12 behaviors)",
        ));
}
...
fn display_repeated_dep_dimmed() {
    ...
    let b_expanded_at_depth1 = lines.iter().any(|l| {
        ...
        (l.starts_with("├── ") || l.starts_with("└── ")) && l.contains("b v1.0.0 (1 behavior)")
    });
    assert!(
...
fn separate_result_and_errors() {
    ...
    let output = minter().arg("validate").arg(&path).assert().success();
