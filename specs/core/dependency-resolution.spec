spec dependency-resolution v2.1.0
...
description
  ...
  avoid re-parsing unchanged files on subsequent runs. NFR files
  are tracked as first-class cache entries alongside specs, ensuring
  that changes to NFR definitions invalidate referencing specs.
...
# Graph cache — NFR tracking

behavior cache-tracks-nfr-files [happy_path]
  "Graph cache stores NFR entries (category, content hash, version, constraint count) alongside spec entries"

  given
    specs/a.spec has nfr refs to performance
    nfr/performance.nfr exists and is valid
    No prior .minter/graph.json exists

  when minter validate --deep specs/

  then
    assert .minter/graph.json contains an nfrs section
    assert nfrs section contains performance entry with content_hash, version, constraint_count


behavior cache-revalidates-on-nfr-change [happy_path]
  "When an NFR file changes, specs referencing that category are revalidated (not skipped)"

  given
    A valid .minter/graph.json exists from a previous run
    specs/a.spec references performance NFR
    nfr/performance.nfr has been modified since the last run

  when minter validate --deep specs/

  then emits stdout
    assert output contains result for a (revalidated, not cached)

  then emits process_exit
    assert code == 0


behavior cache-integrates-new-nfr-files [happy_path]
  "New NFR files are added to the cache on next validation run"

  given
    A valid .minter/graph.json exists from a previous run
    A new file nfr/security.nfr is added to the directory

  when minter validate --deep specs/

  then
    assert .minter/graph.json nfrs section contains security entry


behavior cache-prunes-deleted-nfr-files [edge_case]
  "Deleted NFR files are removed from the cache; specs referencing them are revalidated"

  given
    A valid .minter/graph.json with a performance NFR entry
    nfr/performance.nfr has been deleted from disk

  when minter validate --deep specs/

  then
    assert .minter/graph.json nfrs section does not contain performance
