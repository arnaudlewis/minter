...
use crate::cli::display::{self, CYAN, GREEN, RED, RESET, YELLOW};
use crate::core::discover;
use crate::core::graph::{self, CachedEntry, GraphCache, NfrCachedEntry};
use crate::core::io;
use crate::core::parser;
use crate::core::parser::nfr as nfr_parser;
use crate::core::validation::nfr_semantic;
...
fn file_ext_display(path: &Path) -> &str {
    path.extension().and_then(|e| e.to_str()).unwrap_or("spec")
}
...
pub fn run_watch(path: &Path) -> i32 {
    ...
    } else if path.is_file() && fs::metadata(path).is_err() {
        eprintln!("error: permission denied: {}", path.display());
        return 1;
    }
    ...
    // Check for empty directory (no spec or nfr files found)
    if cache.specs.is_empty() && cache.nfrs.is_empty() {
...
fn is_dependent_of(changed: &Path, watched: &Path, cache: &GraphCache) -> bool {
    let watched_name = watched.file_stem().and_then(|s| s.to_str()).unwrap_or("");
    let changed_name = changed.file_stem().and_then(|s| s.to_str()).unwrap_or("");
...
fn handle_changes(
    ...
    for (name, path) in changed {
        ...
        let dependents: Vec<(String, String)> = cache
            .specs
            .iter()
            .filter(|(n, entry)| *n != name && entry.dependencies.contains(name))
...
fn validate_and_cache_spec(path: &Path, name: &str, dir: &Path, cache: &mut GraphCache) {
    ...
    if ext == "nfr" {
        if !cache.is_nfr_changed(name, &hash) {
            return;
        }
        if let Some((nfr, _valid)) = validate_nfr(path, &source) {
            cache.upsert_nfr(
                name.to_string(),
                NfrCachedEntry {
                    content_hash: hash,
    ...
    if !spec.dependencies.is_empty() {
        ...
        for (i, dep) in spec.dependencies.iter().enumerate() {
            ...
            if let Some(dep_path) = siblings.get(&dep.spec_name) {
                if let Ok(dep_source) = io::read_file_safe(dep_path)
                ...
                println!("{}\u{2717} {} (unresolved)", connector, dep.spec_name);
            }
        }
    }
    let _ = std::io::stdout().flush();
