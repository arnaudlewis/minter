...
impl<'a> Parser<'a> {
    fn parse_when_section(&mut self) -> Result<Action, Vec<ParseError>> {
        ...
        let action_name = line.strip_prefix("when").unwrap().trim().to_string();
        self.advance();
...
impl<'a> Parser<'a> {
    ...
    fn parse_assertion(&mut self) -> Result<Assertion, Vec<ParseError>> {
        ...
        let op_idx = tokens.iter().position(|t| {
            matches!(
                *t,
                "==" | "is_present" | "contains" | "in_range" | "matches_pattern" | ">="
            )
        });
...
fn parse_typed_assertion(
    field: String,
    operator: &str,
    remainder: &str,
) -> Result<Assertion, Vec<ParseError>> {
    match operator {
...
impl<'a> Parser<'a> {
    ...
    fn parse_spec_nfr_section(&mut self) -> Result<Vec<NfrRef>, Vec<ParseError>> {
        ...
        while !self.at_end() {
            ...
            if parts.len() > 1 {
                return Err(self.err("override not allowed in spec-level nfr section"));
            }
...
impl<'a> Parser<'a> {
    ...
    fn parse_one_dependency(&mut self) -> Result<Dependency, Vec<ParseError>> {
        ...
        if parts.len() < 3 || parts[1] != ">=" {
            return Err(self.err(format!("Expected 'depends on {} >= <version>'", spec_name)));
        }
