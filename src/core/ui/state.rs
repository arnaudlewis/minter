...
impl UiState {
    ...
    fn load_integrity(&mut self, working_dir: &Path, specs_dir: &Path, test_dirs: &[PathBuf]) {
        ...
        for tag in &tags {
            ...
            // benchmark is the only special type; all others are behavioral
    ...
    pub fn refresh(&mut self, working_dir: &Path) {
        ...
        self.load_from_dir(working_dir);
    }

    pub fn refresh_spec(&mut self, working_dir: &Path, _spec_path: &Path) {
        // For simplicity, do a full refresh. The spec_path hint is available
        // for future optimization (incremental update).
