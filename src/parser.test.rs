...
behavior with-alias [happy_path]
  ...
  then emits stdout
    ...
    let spec = parse(&input).unwrap();
    match &spec.behaviors[0].preconditions[0] {
        Precondition::Alias { name, entity, properties } => {
...
behavior multi [happy_path]
  ...
  then emits stdout
    ...
    assert!(matches!(&spec.behaviors[0].preconditions[0], Precondition::Prose(_)));
    assert!(matches!(&spec.behaviors[0].preconditions[1], Precondition::Alias { .. }));
}
...
behavior with-returns [happy_path]
  ...
  then returns created item
    ...
    assert_eq!(pc.kind, PostconditionKind::Returns("created item".to_string()));
...
behavior with-side-effect [happy_path]
  ...
  then side_effect
    ...
    let spec = parse(&input).unwrap();
    assert_eq!(spec.behaviors[0].postconditions[0].kind, PostconditionKind::SideEffect);
...
behavior with-ref [happy_path]
  ...
  then returns result
    ...
    let spec = parse(&input).unwrap();
    match &spec.behaviors[0].postconditions[0].assertions[0] {
        Assertion::EqualsRef { field, alias, alias_field } => {
...
behavior bad [happy_path]
  ...
  then emits stdout
    ...
    assert!(msg.contains("given"), "Expected mention of 'given', got: {msg}");
}
...
behavior bad [happy_path]
  ...
  then emits stdout
    ...
    assert!(!errors.is_empty());
    let msg = errors[0].message.to_lowercase();
    assert!(msg.contains("when"), "Expected mention of 'when', got: {msg}");
}
...
",
    ...
    assert!(!errors.is_empty());
    let msg = errors[0].message.to_lowercase();
    assert!(msg.contains("then"), "Expected mention of 'then', got: {msg}");
}
...
fn reject_missing_spec_header() {
    ...
    assert!(!errors.is_empty());
    let msg = errors[0].message.to_lowercase();
    assert!(msg.contains("spec"), "Expected mention of 'spec', got: {msg}");
}
...
";
    ...
    assert!(!errors.is_empty());
    let msg = errors[0].message.to_lowercase();
    assert!(msg.contains("title"), "Expected mention of 'title', got: {msg}");
}
