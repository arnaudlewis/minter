...
static USE_COLOR: std::sync::LazyLock<bool> =
    std::sync::LazyLock::new(|| std::env::var("NO_COLOR").is_err());
...
pub fn nfr_category_count_label(count: usize) -> String {
    if count == 1 {
        "1 nfr category".to_string()
    } else {
        format!("{} nfr categories", count)
    }
}

pub fn nfr_count_label(count: usize) -> String {
    if count == 1 {
        "1 nfr".to_string()
    } else {
        format!("{} nfrs", count)
...
pub struct TreeContext<'a> {
    pub resolved: &'a HashMap<String, crate::core::deps::ResolvedDep>,
...
struct DepthAccumulator<'a> {
    resolved: &'a HashMap<String, crate::core::deps::ResolvedDep>,
...
pub fn compute_shallowest_depths(
    deps: &[Dependency],
    resolved: &HashMap<String, crate::core::deps::ResolvedDep>,
